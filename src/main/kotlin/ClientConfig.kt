...
//    val confirmationTarget: Int
    ...
    init {
        ...
        if (file.exists()) {
            println("The file 'Settings.toml' exists.")
            // Optionally, load settings or do further processing here
        } else {
            println("The file 'Settings.toml' does not exist.")
            // Handle the error or perform alternative actions
        }
        val content = file.readText()
        // Toml.decodeFromString<SettingsToml>(SettingsToml.serializer(), content)

        val parsedTomFile = TomlParser(TomlInputConfig()).parseString(content);
        println("The toml file is " + parsedTomFile.toString());


        val configData = Toml.decodeFromStream<ConfigData>(file.inputStream())
        println("stt.statechainEntity is " + configData.statechainEntity)
